...
slz_gamm$Phonation <- slz_gamm$Phonation %>% 
    factor(levels = c("modal",
                      "breathy", 
                      "checked", 
                      "rearticulated"))
slz_gamm$Speaker <- slz_gamm$Speaker %>% 
    factor()
slz_gamm$Vowel <- slz_gamm$Vowel %>% 
    factor()
slz_gamm$Word <- slz_gamm$Word %>%
    factor()


# Linear gamma model for HNR15
hnr15_linear <- bam(hnr15 ~ Phonation, 
                    data = slz_gamm, method = "ML")
...
slz_gamm$phon_ord <- as.ordered(slz_gamm$Phonation)
contrasts(slz_gamm$phon_ord) <- "contr.treatment"
